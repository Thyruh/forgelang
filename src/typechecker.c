// typecheck_ functions return; on success allowing the program to move on
// to generation
...
static inline void checker_line_error(Tokens* tokens, TokenPos pos, char* err_msg) {
  ...
  while (i < tokens->size && tokens->items[i].pos.line == pos.line) {
    printf(ANSI_COLOR_YELLOW "%s ", token_repr(tokens->items[i]));
    i++;
  }
...
/// Checks if the given symbol from the symbol table at index i is mutable
/// To be called after ident_exists returns the correct index.
static inline bool ident_is_mut(SymbolTable* table, size_t i) {
  return table->items[i].mut;
}
...
static inline void typecheck_stmt(SymbolTable* table, NodeStmt* stmt, ErrorStack* errors) {
  ...
  case STMT_EXIT: 
    {
      if (stmt->stmt_exit->expr->type == EXPR_BIN_EXPR) {
        typecheck_expr();
      }
      ...
      case TERM_IDENT:
        {
          ...
          // Does not fit here obviously, but I need to write the rest of the logic
          // first and move this there
          // if (!ident_is_mut(table, i)) {
          //   checker_line_error(tokens, pos, "Attempting to change an immutable variable");
          // }
        } break;
      default: 
        {
  ...
  case STMT_LET: 
    {
      if (stmt->stmt_let->expr->type == EXPR_BIN_EXPR) {
        typecheck_expr();
      }
      switch (stmt->stmt_let->expr->value.term->type) {
      // rhs cases
      case TERM_INT_LIT: 
        {
