...
__attribute__((warn_unused_result)) static inline Token peek(Parser* p, size_t offset) {
...
//
static inline void parser_line_error(Parser* p, const char* err_msg, u8 offset) {
...
static inline Token try_consume(Parser* p, TokenType type, const char* err_msg) {
  if (type == semi) {
    if (peek(p, 0).type == semi) {
      return consume(p);
    }
    else {
      parser_line_error(p, err_msg, 1);
      exit(1);
    }
  }
  ...
  else parser_line_error(p, err_msg, 1);
  exit(1); // to silence void return warning
}

static inline void symbol_table_append(SymbolTable* table, const char* ident,
                                       TokenType type, bool mut) 
{
  Symbol sym = {0};
  sym.type = type;
  sym.ident = ident;
...
static inline NodeExpr* parse_expr(Parser* p, int min_prec) {
  ...
  while (1) {
    Token op = peek(p, 0);
    int prec = get_prec(op.type);
    if (prec < min_prec)
      break;

    Token op_token = consume(p);
    ...
    bin_expr->lhs = lhs;
    bin_expr->rhs = rhs;
    bin_expr->op = op_token;
    if      (op_token.type == plus)   bin_expr->type = EXPR_ADD;
    else if (op_token.type == star)   bin_expr->type = EXPR_MULTI;
    else if (op_token.type == fslash) bin_expr->type = EXPR_DIVIDE;
    else if (op_token.type == minus)  bin_expr->type = EXPR_SUBTR;
...
static inline NodeStmt* parse_stmt(Parser* p) {
  ...
  if ((peek(p, 0).type == mut || peek(p, 0).type == const_) 
    ...
    symbol_table_append(p->table, stmt_let->ident.value, stmt_let->type, stmt_let->mut);
  } 
    ...
    parser_line_error(p, "Invalid statement", 0);
  }
  return stmt;
}
