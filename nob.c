...
static bool compile_object(const char *c_path, const char *o_path, bool debug)
{
    Cmd cmd = {0};
    cmd_append(&cmd, "cc");
    if (debug) {
...
static bool link_target(bool debug)
{
    Cmd cmd = {0};
    cmd_append(&cmd, "cc");
    if (debug) {
...
static bool build(bool debug)
{
    ...
    for (size_t i = 0; i < ARRAY_LEN(src); ++i) {
        ...
        if (rebuild) {
            ...
            if (!compile_object(c, o, debug)) return false;
            any_rebuilt = true;
        } else {
